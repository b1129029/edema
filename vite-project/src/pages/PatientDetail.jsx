...
const PatientDetail = ({ isLoggedIn, onLoginToggle }) => {
  ...
  if (selectedEntry?.photo_path) {
    console.log("Image Source:", `https://edema.ngrok.app${selectedEntry.photo_path.trim()}`);
  }
}, [selectedEntry]);
  ...
  const fetchData = async () => {
    ...
    try {
      // 1. 先取得病患基本資料
      const patientRes = await fetch(`https://edema.ngrok.app/api/patients/${id}`)
      if (!patientRes.ok) throw new Error('Failed to fetch patient')
      const patientData = await patientRes.json()
      setPatient(patientData)

      // 2. 接著取得 foot_data
      const footRes = await fetch(`https://edema.ngrok.app/api/patients/${id}/foot_data`)
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
        try {
        const res = await fetch(`https://edema.ngrok.app/api/patients/${patient.id}`, {
        method: 'PUT',
                        ...
                        <button
                          onClick={() => {
                            if (window.confirm('Are you sure you want to delete this record?')) {
                              console.log(item.id)
                              fetch(`https://edema.ngrok.app/api/patients/${id}/foot_data/${item.id}`, {
                                method: 'DELETE',
                              })
                                .then((res) => {
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-4xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="border rounded overflow-hidden flex items-center justify-center bg-gray-50 h-full min-h-[200px]">
                {selectedEntry.photo_path ? (
                  <img 
                    src={`https://edema.ngrok.app${selectedEntry.photo_path.trim()}`}
                    alt="Foot photo"
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    const res = await fetch(`https://edema.ngrok.app/api/foot_data/${selectedEntry.id}`, {
                      method: 'PUT',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
